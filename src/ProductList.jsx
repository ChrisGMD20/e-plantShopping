...
function ProductList({handleGetStarted}) {
  ...
  const { items, cartCount, cartTotalAmount } = useSelector(
    (state) => state.cart
  );
  ...
  function handleAddToCart(plant) {
    ...
    if (!addedToCart.includes(plant.name)) {
      setAddedToCart((prev) => [...prev, plant.name]);
    }
  }
  function handleIncrement(name) {
    dispatch(updateQuantity({ name, quantity: 1 }));
  }
  function handleRemove(name){
    dispatch(removeItem(name))
          setAddedToCart((prev) => {
        const list = prev.filter((item) => item !== name);
        return list;
      });
  }
  function handleDecrement(name, quantity) {
    if (quantity === 1) {
      dispatch(removeItem(name));
      setAddedToCart((prev) => {
        const list = prev.filter((item) => item !== name);
    ...
    <div>
      <div className="navbar" style={styleObj}>
        ...
        <div style={styleObjUl}>
          ...
          <div onClick={() => setCartIsVisible((prev) => !prev)}>
            {" "}
            <span style={{ position: "absolute", right: "42px", top: "38px" }}>
              {cartCount}
            </span>
            <a href="#" style={styleA}>
              <h1 className="cart">
      ...
      {!cartIsVisible ? (
        <div className="product-grid">
            {plantsArray.map((products) =>(
                <>
            <h2 className="plant_heading">{products.category}</h2>
            <ul className="product-list">
              {products.plants.map((plant) => (
                <li key={plant.name} className="product-card">
                  ...
                  <button
                    onClick={() =>
                      handleAddToCart({
                        ...
                        total: parseInt(plant.cost.replace("$", "")),
                        quantity: 1,
                      })
                    }
                    className="product-button"
                    style={addedToCart.includes(plant.name)?{backgroundColor: "grey"}:undefined}
                  >
                    Add to Cart
                  </button>
                </li>
        ...
        <div className="cart-modal">
          <h3 className="">Total Cart Amount: ${cartTotalAmount}</h3>
          {items.map((item) => (
            <section key={item.name} className="cart-card">
              ...
              <div className="cart-card-details">
                ...
                <p className="bold">Total: ${item.total}</p>
                <button onClick={()=>handleRemove(item.name)} className="delete">Delete</button>
              </div>
            </section>
          ))}
          <button
